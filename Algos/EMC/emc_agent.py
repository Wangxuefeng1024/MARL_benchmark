from Algos.EMC.network import DRQN, VDN
...
class EMC:
    def __init__(self, args):
        ...
        # network
        self.Target_Q = DRQN(self.n_states, args)
        self.Predictor = DRQN(self.n_states, args)
        self.VDN = VDN(self.n_agents, args)
        self.Q_network = deepcopy(self.Target_Q)
        if self.args.cuda:
            self.Target_Q.cuda()
            self.Predictor.cuda()
            self.VDN.cuda()
            self.Q_network.cuda()
        # nothing to say
        self.save_path = self.args.result_dir + '/' + args.algo + '/' + args.env
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, maven_z=None, evaluate=False):
        ...
        # get q value
        q_value, self.eval_hidden[:, agent_num, :] = self.Target_Q(inputs, hidden_state)
        # choose action from q value
        if np.random.uniform() < epsilon:
            action = np.random.choice(np.ones(self.n_agents))  # action是一个整数
        else:
    ...
    def _choose_action_from_softmax(self, inputs):
        """
        ...
        prob = ((1 - self.epsilon) * prob + torch.ones_like(prob) * self.epsilon / self.n_actions)
