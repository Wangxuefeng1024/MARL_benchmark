import torch
import os
from Algos.iql.network import CNN_Actor, Actor
...
class im_IQL:
    def __init__(self, env, args):
        ...
        # 神经网络
        self.eval_rnn = CNN_Actor(input_shape, args)
        self.target_rnn = CNN_Actor(input_shape, args)
        self.args = args
        self.writer = SummaryWriter(log_dir='runs/' + args.algo + "/" + args.scenario + "/" + str(args.n_agents))
        if self.args.cuda:
            self.eval_rnn.cuda()
            self.target_rnn.cuda()
    ...
    def learn(self, train_step):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        '''
        在learn的时候，抽取到的数据是四维的，四个维度分别为 1——第几个episode 2——episode中第几个transition
        3——第几个agent的数据 4——具体obs维度。因为在选动作时不仅需要输入当前的inputs，还要给神经网络输入hidden_state，
        hidden_state和之前的经验相关，因此就不能随机抽取经验进行学习。所以这里一次抽取多个episode，然后一次给神经网络
        传入每个episode的同一个位置的transition
        '''
        episode_num = self.args.episode_limit
        self.init_hidden(episode_num)
        for key in self.buffer.keys():  # 把batch里的数据转化成tensor

            self.buffer[key]  = torch.tensor(self.buffer[key], dtype=torch.float32)
        u, r = self.buffer['u'], self.buffer['r'].repeat(1, 1, self.n_agents)
        # mask = (1 - batch["padded"].float()).repeat(1, 1, self.n_agents)  # 用来把那些填充的经验的TD-error置0，从而不让它们影响到学习
    ...
    def generate_episode(self, episode_num=None, evaluate=False):
        ...
        while not done and step < self.args.episode_limit:
            # time.sleep(0.2)
            obs = [self.env.observe('piston_'+str(i)) for i in range(self.args.n_agents)]
            # state = self.env.get_state()
            actions, avail_actions, actions_onehot = [], [], []
            for agent_id in range(self.n_agents):
                # avail_action = self.env.get_avail_agent_actions(agent_id)
                action = self.choose_action(obs[agent_id], last_action[agent_id], agent_id, epsilon)
            ...
            self.buffer['o'].append(obs)
            # self.buffer['s'].append(state)
            self.buffer['u'].append(np.reshape(actions, [self.n_agents, 1]))
            self.buffer['u_onehot'].append(actions_onehot)
            # avail_u.append(avail_actions)
            self.buffer['r'].append([reward])
            # self.buffer['terminate'].append([terminated])
            # padded.append([0.])
        ...
        self.buffer['o'].append(obs)
        # self.buffer['s'].append(state)
        self.buffer['o_next'] = self.buffer['o_next'][1:]
        self.buffer['s_next'] = self.buffer['s_next'][1:]
        self.buffer['o'] = self.buffer['o'][:-1]
        self.buffer['s'] = self.buffer['s'][:-1]
        # get avail_action for last obs，because target_q needs avail_action in training
        self.writer.add_scalar(tag='agent/reward', global_step=episode_reward, scalar_value=episode_num)
    ...
    def evaluate(self):
        ...
        episode_rewards = 0
        for epoch in range(self.args.evaluate_epoch):
            episode_reward, win_tag, _ = self.generate_episode(epoch, evaluate=True)
            episode_rewards += episode_reward
            if win_tag:
                win_number += 1
        return win_number / self.args.evaluate_epoch, episode_rewards / self.args.evaluate_epoch
    ...
    def choose_action(self, obs, last_action, agent_num, epsilon):
        inputs = obs.copy()
        avail_actions_ind = np.nonzero(self.args.n_actions)[0]  # index of actions which can be choose
        ...
        q_value = self.eval_rnn(inputs, last_action, agent_id)
    ...
    def train(self, train_step, epsilon=None):  # coma needs epsilon for training

        # different episode has different length, so we need to get max length of the batch
        self.learn(train_step)
        if train_step > 0 and train_step % self.args.save_cycle == 0:
            self.save_model(train_step)

    def get_q_values(self, max_episode_len):
        episode_num = self.args.episode_limit
        q_evals, q_targets = [], []
        for transition_idx in range(max_episode_len):
            inputs, inputs_next = self._get_inputs(batch, transition_idx)  # 给obs加last_action、agent_id
