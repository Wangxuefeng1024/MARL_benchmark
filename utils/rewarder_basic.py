...
def calc_reward(rew, prev_obs, obs, timestep):
    ...
    close_reward = -0.1*(abs(1-ball_x) + abs(0-ball_y))
    # penalty reward for timestep:
    penalty_reward = -0.01*timestep
    # reward = 2.0 * win_reward + 20.0 * rew + 0.06 * ball_position_r + yellow_r + close_reward
    # reward = 20.0 * rew + close_reward + penalty_reward
    reward = 20.0 * rew + close_reward
