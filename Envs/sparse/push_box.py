import numpy as np
import copy
from Envs.sparse.rooms import Entity
import gym
...
class PushBox(object):
    ...
    def reset(self):
        ...
        if self.checkpoint:
            self.cur_checkpoint = {'dir':-1, 'dist':0}
        return self.revise_obs(np.array([self.agents[0].x, self.agents[0].y, self.agents[1].x, self.agents[1].y, self.box.x, self.box.y]))
    ...
    def step(self, action):
        ...
        return self.revise_obs(np.array(obs)), rew, self.done
    ...
    def revise_obs(self, obs):
        obs_1 = [obs[0],obs[1],obs[4],obs[5]]
        obs_2 = [obs[2],obs[3],obs[4],obs[5]]
        return [obs_1, obs_2]
