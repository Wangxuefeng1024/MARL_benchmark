...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        dist = [abs(other_pos[i][0])+abs(other_pos[i][1]) for i in range(len(other_pos))]
        index_min = dist.index(min(dist))
        dist[index_min] = 999
        index_second = dist.index(min(dist))
        dist[index_second] = 999
        index_third = dist.index(min(dist))
        x = np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + entity_pos)
        x_3 = np.concatenate([x, other_pos[index_second], other_pos[index_third]])
        return x_3
