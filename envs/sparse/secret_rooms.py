import numpy as np
import copy
from Envs.sparse.rooms import Rooms, Entity, Door
import gym
TOP = 0
BOT = 1
LEFT = 2
...
class SecretRooms(Rooms):
    ...
    def reset(self):
        ...
        self.ckpts = {'left_switch': [[False, bin] for bin in self.ckpt_bins['left_switch']],
                      'door': [[False, bin] for bin in self.ckpt_bins['door']],
                      'right_switch': [[False, bin] for bin in self.ckpt_bins['right_switch']]}
        return self.revise_obs(np.array([self.agents[0].x, self.agents[0].y, self.agents[1].x, self.agents[1].y,
    ...
    def revise_obs(self, obs):
        obs_1 = [obs[0],obs[1],obs[4],]
        obs_2 = [obs[2],obs[3],obs[4]]
        return [obs_1, obs_2]