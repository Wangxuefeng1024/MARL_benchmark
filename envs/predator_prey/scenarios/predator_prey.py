...
class Scenario(BaseScenario):
    def make_world(
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.accel = 5.0
        # add landmarks
        world.preys = [Agent() for i in range(args.n_agents)]
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            agent.state.p_pos = np.random.uniform(
                -world.range_p, +world.range_p, world.dim_p
            )
                    ...
                    while True:
                        if (
                            np.sqrt(
                                np.sum(
                                    np.square(
                                        prey.state.p_pos - world.preys[j].state.p_pos
                                    )
                                )
                            )
                            > 0.00
    ...
    def benchmark_data(self, agent, world):
        ...
        for p in world.preys:
            ...
            dists = [
                np.sqrt(np.sum(np.square(a.state.p_pos - p.state.p_pos)))
    ...
    def observation(self, agent, world):
        ...
        for entity in world.preys:  # world.entities:
            ...
            dis_lm_n.append(
                np.sqrt(np.sum(np.square(agent.state.p_pos - entity.state.p_pos)))
            )
        ...
        for other in world.agents:
            if other is agent:
                continue
            dis_agent_n.append(
                np.sqrt(np.sum(np.square(agent.state.p_pos - other.state.p_pos)))
            )
        ...
        return np.concatenate(
            [agent.state.p_vel] + [agent.state.p_pos] + near_lm_pos + near_agent_pos
        )
