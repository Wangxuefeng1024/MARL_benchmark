...
import torch
import argparse
from Algos.iac.iac_agent import im_IAC
...
def main(args):
    ...
    # reduce the observation image to gray mode
    ...
    env = ss.frame_stack_v1(env, 3)
    args.n_actions = 1
    args.obs_shape = (env.observation_space('piston_0').high.shape[2],env.observation_space('piston_0').high.shape[0],env.observation_space('piston_0').high.shape[1])

    torch.manual_seed(args.seed)

    # if args.tensorboard and args.mode == "train":
    writer = SummaryWriter(log_dir='runs/' + args.algo + "/" + "pistonball/"+str(args.n_agents))

    model = im_IAC(env, args)
    episode = 0
    while True:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--episode_length', type=int, default=50, help='rnn dimension')
    ...
    parser.add_argument('--continuous', type=bool, default=True, help='whether to use two hyper layers')
