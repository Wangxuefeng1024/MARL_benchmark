...
from network.qatten import DRQN, QattenNet
...
class Qatten:
    def __init__(self, env, args):
        ...
        self.eval_qmix_net = QattenNet(args)  # 把agentsQ值加起来的网络
        self.target_qmix_net = QattenNet(args)
        if self.args.cuda:
            self.eval_rnn.cuda()
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        q_total_eval = self.eval_qmix_net(q_evals, s, o)
        q_total_target = self.target_qmix_net(q_targets, s_next, o_next)
