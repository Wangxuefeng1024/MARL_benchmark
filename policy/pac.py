...
from network.pac import Actor, Critic, QMixingNetwork, QstarNetwork, IBFComm
...
class PAC:
    def __init__(self, env, args):
        ...
        self.eval_pagent = Critic(input_shape, args)
        if self.args.cuda:
            self.eval_rnn.cuda()
        ...
        if args.optimizer == "RMS":
            ...
            self.qstar_optimiser = torch.optim.RMSprop(self.target_qmix_net.parameters(), lr=args.lr)
        else:
            ...
            self.qstar_optimiser = torch.optim.Adam(self.target_qmix_net.parameters(), lr=args.lr)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        q_i_mean_negi_mean = torch.sum(actions_prob * (self.alpha * log_prob_pi - q_evals),
                                       dim=-1)  # (1,60,3) # TODO
        ...
        # wqmix: weighting here
        ws = torch.ones_like(td_error) * self.w_to_use
        ...
        #######################
        # Qstar loss
        q_star_loss = (masked_td_error ** 2).sum() / mask.sum()
        ...
        KL_loss = torch.distributions.kl_divergence(torch.distributions.Normal(mu_out, sigma_out),
                                                    torch.distributions.Normal(self.s_mu,
                                                                               self.s_sigma)).sum() / mask.sum()

        # Entropy loss
        entropy_loss = -torch.distributions.Normal(self.s_mu, self.s_sigma).log_prob(
        ...
        self.qstar_optimiser.zero_grad()
        q_star_loss.backward(retain_graph=True)
        torch.nn.utils.clip_grad_norm_(self.eval_parameters, self.args.grad_norm_clip)
        self.qstar_optimiser.step()
